...
$id_kata = mysqli_query($mysql, "SELECT * from katalog where katalog='$katalog'");
$id_katalo = mysqli_fetch_array($id_kata);
...
if($_FILES['gambar']['name']){
	...
	if(!$_FILES['gambar']['error']){
		...
		if($valid_file == true)
		{
			...
			// simpan ke database
			mysqli_query($mysql, "INSERT into buku set judul='$judul',id_kategori='$id_kategori',id_katalog='$id_katalog',pengarang='$pengarang',penerbit='$penerbit',hal='$halaman',harga='$harga',deskripsi='$deskripsi',tanggal='$tanggal',gambar='$nama_file_baru'");
